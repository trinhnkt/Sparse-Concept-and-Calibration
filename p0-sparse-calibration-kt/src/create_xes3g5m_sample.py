...
def main():
    ...
    sample_output = sample_dir / "xes3g5m_sample.csv"
    
    if not train_path.exists():
        print(f"File not found at {train_path}")
        return
        
    print(f"Reading sequence data from {train_path}...")
    # Load first 300 sequences (enough to have stable users and concept stratification)
    df_seq_train = pd.read_csv(train_path, nrows=200)
    df_seq_test = pd.read_csv(test_path, nrows=100) if test_path.exists() else pd.DataFrame()
    ...
    df_flat.to_csv(sample_output, index=False)
    print(f"Saved canonical interaction sample to {sample_output} ({len(df_flat)} rows)")
