...
│
├── jedm_upload_folder/                 # Mã nguồn LaTeX định dạng tạp chí JEDM (Journal of Educational Data Mining)
│   ├── main_jedm.tex                   # Bản thảo chính thức JEDM (Có thông tin tác giả)
│   └── main_jedm_anonymous.tex         # Bản thảo giấu tên JEDM (Double-blind review)
│
├── springer_upload_folder/             # Mã nguồn LaTeX định dạng Springer Traditional
│   ├── main_springer_traditional.tex   # Bản thảo Springer
│   └── main_springer_traditional_anonymous.tex # Bản thảo Springer giấu tên
│
└── paper/                              # Bản thảo khoa học định dạng Springer Compact và lưu trữ chung
    ├── sections/                       # Tệp nguồn nội dung LaTeX chia theo từng chương
    ├── tables/                         # Các bảng biểu khoa học sinh tự động từ thực nghiệm
    ├── figures/                        # Sơ đồ và biểu đồ định dạng Vector
    ├── main_springer_compact.tex       # Bản thảo định dạng Springer Compact (rút gọn)
    └── references.bib                  # Cơ sở dữ liệu trích dẫn khoa học (BibTeX)
```
...
### 📄 5.1. Bản thảo Đa Định Dạng (Multi-format LaTeX Candidates)
Dự án cung cấp bộ mã nguồn LaTeX đã được biên dịch và kiểm định (Zero Errors/Warnings) cho các tạp chí hàng đầu:
* **Tạp chí JEDM (Journal of Educational Data Mining):** Lưu trữ trong thư mục `jedm_upload_folder/`. Bao gồm bản đầy đủ (`main_jedm.tex`) và bản giấu tên (`main_jedm_anonymous.tex`) theo đúng cấu trúc tiêu chuẩn (Acknowledgements -> Declaration -> References -> Appendix).
* **Springer Traditional Format:** Lưu tại `springer_upload_folder/`. Hỗ trợ cấu trúc cổ điển của hệ thống nộp bài Springer (`main_springer_traditional.tex`).
* **Springer Compact Format:** Lưu tại `paper/main_springer_compact.tex`.

### 📊 5.2. Các Bảng Biểu LaTeX Khoa Học Tự Động (LaTeX Tables)
Các bảng biểu tự động chèn vào bài báo được đặt tại `jedm_upload_folder/tables/` và `springer_upload_folder/tables/`:
* **Table 2 & 3 (Overall Results & DeLong):** Kết quả tổng hợp và kiểm định thống kê.
* **Table 4 (Performance by Strata):** Kết quả kiểm định phân tầng KC kèm #KCs và #Events để theo dõi metric instability.
* **Table 5 (Calibration Breakdown & Cold-Start Temporal):** Đánh giá hiệu chuẩn (ECE/Brier) và khả năng suy quát temporal cold-start. Bảng được tách riêng theo 3 dataset (ASSISTments, Junyi, XES3G5M) để hiển thị thân thiện trên JEDM.

### 📉 5.3. Trực quan hóa & Báo cáo Chẩn đoán (Figures & Reports)
* **Biểu đồ độ tin cậy (Reliability Diagrams):** Lưu trữ trong [results/figures/reliability_per_bucket/](file:///c:/TRINH/Sparse-Concept and Calibration/sparse-calibration-kt/results/figures/reliability_per_bucket/) - trực quan hóa rõ nét sai số hiệu chuẩn đối với từng tầng KCs từ dense đến sparse (ví dụ: Junyi Temporal Dense/Very Sparse).
* **Báo cáo Chẩn đoán & Thẩm định quan trọng:**
  * [table_figure_update_report.md](file:///c:/TRINH/Sparse-Concept and Calibration/sparse-calibration-kt/results/reports/table_figure_update_report.md): Chi tiết quá trình cập nhật các bảng biểu LaTeX và biểu đồ sau rerun.
  * [final_presubmission_consistency_check.md](file:///c:/TRINH/Sparse-Concept and Calibration/sparse-calibration-kt/results/reports/final_presubmission_consistency_check.md): Checklist tổng thể kiểm tra độ nhất quán toàn bộ paper trước submission.
  * [latex_compile_check_report.md](file:///c:/TRINH/Sparse-Concept and Calibration/sparse-calibration-kt/results/reports/latex_compile_check_report.md): Báo cáo kiểm định biên dịch LaTeX và sinh file PDF thành công.
  * [classical_baseline_decision_report.md](file:///c:/TRINH/Sparse-Concept and Calibration/sparse-calibration-kt/results/reports/classical_baseline_decision_report.md): Phân tích chi tiết lỗi pyBKT và cơ sở khoa học của việc chuyển sang IRT 1PL.
  * [final_submit_candidate_validation_report.md](file:///c:/TRINH/Sparse-Concept and Calibration/sparse-calibration-kt/results/reports/final_submit_candidate_validation_report.md): Báo cáo thẩm định tính nhất quán và độ tin cậy của pipeline ở phiên bản trước.
...
* Nhấn mạnh **metric instability** do giới hạn kích thước mẫu (sample size) thay vì đánh giá hiệu năng chênh lệch.

### 🔍 6.4. Kiểm Toán Label-Alignment (Temporal Split)
Trong quá trình thẩm định temporal splits, chúng tôi đã phát hiện lỗi mất đồng bộ (misalignment) giữa file `predictions` và nhãn `y_true`. Lỗi này làm AUC của KCs thuộc nhóm "warm" (có lịch sử huấn luyện) bị rơi xuống ngưỡng ngẫu nhiên (`~0.50`). 
Quy trình **Sanity check & Label-Alignment correction** đã được thực thi và phục hồi hoàn toàn năng lực học tập của các mô hình Deep KT. Toàn bộ mã nguồn phục hồi và kiểm định đều được cung cấp công khai.

### 💡 6.5. Phân Tích Hiện Tượng Đa Kỹ Năng (Multi-Skill Artifacts) trên XES3G5M
Qua việc đối chiếu item-level, chúng tôi lý giải hiện tượng "AUC trên sparse concept cao bất thường" ở bộ dữ liệu XES3G5M. Các KCs rất hiếm (`sparse`) thường đồng xuất hiện cùng KCs cực kỳ phổ biến (`dense`) trong cùng một câu hỏi. Deep KT models thực chất đã lợi dụng tín hiệu từ dense KC thay vì học được quy luật của sparse KC. Khám phá này củng cố quan điểm phải chẩn đoán dựa trên từng "strata" kết hợp calibration thay vì phụ thuộc AUC tổng cục.
